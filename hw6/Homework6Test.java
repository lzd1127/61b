...
public class Homework6Test {
   ...
   **/
  private static SimpleBoard testBoard(){
    SimpleBoard board = new SimpleBoard();
    for (int y = 0; y < 8; y++) {
      for (int x = 0; x < 8; x++) {
        int value = 0;
  ...
  public static void main(String[] args) {
    ...
    table.insert(testBoard(),new Integer(111111));
    try{
     // System.out.println(table.compFunction(randomBoard().hashCode()));
      System.out.println(table.find(testBoard()).value());
      System.out.println(table.remove(testBoard()).value());
