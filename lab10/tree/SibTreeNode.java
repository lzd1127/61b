...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    int count = this.children();
    SibTreeNode child = new SibTreeNode(this.myTree,item);
    if (count ==0){
      this.firstChild = child;
      child.parent = this;
    }else if (c >count){
     SibTreeNode tmp = (SibTreeNode)child(count);
     tmp.nextSibling = child;
     child.parent = this;
    }else if (c <=1){
      SibTreeNode tmp = (SibTreeNode)child(1);
      this.firstChild =child;
      child.nextSibling = tmp;
      child.parent = this;
    }else{
      SibTreeNode tmp = (SibTreeNode)child(c-1);
      child.nextSibling = tmp.nextSibling;
      tmp.nextSibling = child;
      child.parent = this;
    }
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    if (this.myTree.size ==1){
      valid = false;
      myTree.size--;
      return;
    }else if (firstChild ==null){
      SibTreeNode tmp = (SibTreeNode)this.parent();
      SibTreeNode curr = tmp.firstChild;
      if (curr ==this)
        tmp.firstChild = curr.nextSibling;
      else{
